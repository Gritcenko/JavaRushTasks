...
addToList(Runnable), который добавляет в список list элемент, полученный в качестве входного параметра, и метод runList(), который вызывает метод run() у каждого элемента списка list.
...
public class Solution {
    ...
    public static void runList() {
        for (Runnable element: list) {
            element.run();//напишите тут ваш код
        }
    }
}